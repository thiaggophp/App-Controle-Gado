...
export default function Modal({open,onClose,title,children}){
  ...
  // Toque real no backdrop: touchstart + touchend na mesma área → fecha
  // Click sintético do Android/iOS ao voltar da câmera: só dispara "click" (sem touchstart)
  // → isTouch=true → onClick ignorado → modal não fecha
  ...
  return(<div
    ...
    onClick={onBdClick}
    style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,.8)",display:"flex",alignItems:"flex-end",justifyContent:"center",backdropFilter:"blur(4px)",animation:"fadeIn .2s ease"}}>
    <div onClick={e=>e.stopPropagation()} style={{background:"#0e1a0e",borderRadius:"28px 28px 0 0",padding:"20px 20px 36px",width:"100%",maxWidth:480,maxHeight:"90vh",overflowY:"auto",animation:"slideUp .25s ease",border:"1px solid rgba(255,255,255,0.07)",borderBottom:"none"}}>
